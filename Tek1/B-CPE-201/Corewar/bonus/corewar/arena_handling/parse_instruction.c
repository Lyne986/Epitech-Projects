...
int get_number_from_n_bytes(corewar_t *core, int type, int *i)
{
    ...
    for (int index = 0; index < n; index++) {
        my_strcat_from(core->hex, core->arena[index + *i].hex, index * 2);
    }
    (*i) += n;
...
void execute_args(corewar_t *core, champion_t *champ, instruction_t *function)
{
    ...
    function->code != 12 && function->code != 15) {
        add_args(function, core, &champ->next_pc);
    } else {
        function->coding_byte = 0;
        if (op.nbr_cycles >= 20 && op.type[0] == T_DIR) {
            function->args[0] =
            get_number_from_n_bytes(core, T_IND,
            &champ->next_pc);
        } else {
            function->args[0] =
            get_number_from_n_bytes(core, op.type[0],
            &champ->next_pc);
        }
