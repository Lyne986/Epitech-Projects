/*
** EPITECH PROJECT, 2020
** my.h
** File description:
** my.h
*/

#include <stddef.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <stdarg.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
#include <stdio.h>
#include <fcntl.h>
#include <stdbool.h>
#include <time.h>

#include "garbage_collector.h"
#include "op.h"

char *my_itoa(int number);
char *nbr_to_base(unsigned int nbr, char *base);
char *my_unsigned_memset(unsigned char *str, char c, int size);
int my_put_long(long int nb);
char *my_strdup(char const *src);
void my_putchar(char c);
int my_isneg(int nb);
int my_put_nbr(int nb);
int my_put_nbr_un(unsigned int nb);
void my_swap(int *a, int *b);
int my_putstr(char const *str);
int my_strlen(char const *str);
long int my_get_longnbr(char const *str);
long int my_long_power_rec(long int nb, int p);
int my_getnbr(char const *str);
void my_sort_int_array(int *tab, int size);
int my_compute_power_rec(int nb, int power);
int my_compute_square_root(int nb);
int my_is_prime(int nb);
int my_find_prime_sup(int nb);
char *my_strcpy(char *dest, char const *src);
char *my_strncpy(char *dest, char const *src, int n);
char *my_revstr(char *str);
char *my_strstr(char *str, char const *to_find);
int my_strcmp(char const *s1, char const *s2);
int my_strncmp(char const *s1, char const *s2, int n);
char *my_strupcase(char *str);
char *my_strlowcase(char *str);
char *my_strcapitalize(char *str);
int my_str_isalpha(char const *str);
int my_str_isnum(char const *str);
int my_str_islower(char const *str);
int is_number(char *str);
int my_str_isupper(char const *str);
int my_str_isprintable(char const *str);
int my_showstr(char const *str);
int my_showmem(char const *st, int size);
char *my_strcat(char *dest, char *src);
char *my_strncat(char *dest, char const *src, int nb);
char *to_str(int nb);
char **my_str_to_word_array(char const *str, char *delim);
char *my_memset(char *str, char c, int size);
char **split_string_with_delim(char const *str, char *delim);
int is_delim(char c, char const *delim);
int my_printf(char *str, ...);
void my_strcat_from(char *dest, char *src, int n);